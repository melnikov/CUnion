...
- (void)loadView
{
    ...
    [_toolbar setBackgroundImage:[UIImage imageNamed:@"top_bar_bg.png"]
    ...
    // setup thumbs view
    _thumbsView.backgroundColor					= [UIColor colorWithPatternImage:[UIImage imageNamed:@"bg.png"]];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            ...
            _useThumbBtn = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 88.0f, 30.0f)];
            [_useThumbBtn setBackgroundImage:[UIImage imageNamed:@"barBtn.png"] forState:UIControlStateNormal];
            [_useThumbBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_useThumbBtn setTitle:@"Показать все" forState:UIControlStateNormal];
            _useThumbBtn.titleLabel.font = [UIFont boldSystemFontOfSize:12.0f];
            [_useThumbBtn addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchUpInside];
            
            [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithCustomView:_useThumbBtn] animated:NO];
        }
        else {
...
- (void)positionInnerContainer
{
	CGRect screenFrame = CGRectMake(0.0f, 0.0f, _gallerySize.width, _gallerySize.height);
...
- (void)updateTitle
{
	[self setTitle:[NSString stringWithFormat:@"%i из %i", _currentIndex+1, [_photoSource numberOfPhotosForPhotoGallery:self]]];
}
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [_useThumbBtn setTitle:@"Готово" forState:UIControlStateNormal];
...
- (void)didReceiveMemoryWarning {
	...
	FTLog(@"[FGalleryViewController] didReceiveMemoryWarning! clearing out cached images...");
