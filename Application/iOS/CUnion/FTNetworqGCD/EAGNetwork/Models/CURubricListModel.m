...
#define DB_CLASS_FILE @"rubric_list.postkomsg.com"

@implementation CURubricListModel

- (void)_initData
{
    _cache = [[CUServerManager sharedManager] getDBCache];
    _loader = [[CUServerManager sharedManager] makeLoader:CUServerLoaderRubricList];
...
- (void)_loadFromServer:(id)data_
{
    ...
    @autoreleasepool {
        [_loader setParameter:[NSString stringWithFormat:@"%d", _parentID] forKey:@"rubric_id"];
        [_loader load];
    }
}
